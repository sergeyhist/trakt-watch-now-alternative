...
function awBlock(type, attributes, styles) {
  ...
  block.onclick = (event) => {
    ...
    awModal.addEventListener("click", (e) => {
      !awBlock.contains(e.target) && closeModal(awModal);
    });
    awModal.addEventListener("keydown", (e) => {
...
function createLB(type, items) {
  const toggleList = (list, e) => {
    if (
      ...
      !list.querySelector(".aw-select").classList.contains("aw-hidden")
    ) {
      list.querySelector(".aw-select").classList.add("aw-hidden");
      setTimeout(() => {
        list.querySelector(".aw-title").style.borderRadius = "3px";
      }, 100);
    } else {
      list.querySelector(".aw-select").classList.remove("aw-hidden");
      setTimeout(() => {
        list.querySelector(".aw-title").style.borderRadius = "3px 3px 0 0";
      }, 100);
    }
  };
  ...
  if (items) {
    for (let item of items) {
      ...
      option.onclick = () => {
        selectOption(awTitle);
      };
      option.onkeydown = (e) => {
        e.key == "Enter" && selectOption(awTitle);
      };
  ...
  document.addEventListener("click", (e) => {
    toggleList(awSearchOption, e);
  });
  document.addEventListener("keydown", (e) => {
    e.key == "Enter" && toggleList(awSearchOption, e);
  });
}
...
function updateLB(type, items) {
  ...
  for (let item of items) {
    ...
    option.onclick = () => {
      selectOption(listBox.querySelector(".aw-title"));
    };
    option.onkeydown = (e) => {
      e.key == "Enter" && selectOption(listBox.querySelector(".aw-title"));
    };
  }

  if (
    listBox.querySelector("span").textContent == "" ||
    listBox.querySelector("span").textContent == "Loading..."
  ) {
    listBox.querySelector("span").textContent = items[0];
  }
}

function updateTitle() {
  const awSearchString = document.querySelector(".aw-search-string");
  const selectedAlias = document
    .querySelector("#aw-aliases")
    .querySelector("span").textContent;

  awSearchString.value = awSearchString.value.replace(
    aw_data.title,
    selectedAlias
  );
...
function updateInfo() {
  const awSearchString = document.querySelector(".aw-search-string");
  const selectedInfo = document
    .querySelector("#aw-info")
    .querySelector("span").textContent;
  ...
  switch (selectedInfo) {
    ...
    case "Season":
      ...
      break;
    case "Episode":
      awSearchString.value =
        aw_data.title + " s" + aw_data.season + "e" + aw_data.episode;
      break;
    case "Year":
    ...
    case "Season+Year":
      awSearchString.value =
        aw_data.title + " s" + aw_data.season + " " + aw_data.year;
      break;
    case "Episode+Year":
      awSearchString.value =
        aw_data.title +
    ...
    case "Absolute+Year":
      awSearchString.value =
        aw_data.title + " " + aw_data.abs_episode + " " + aw_data.year;
  }
}
...
function updateOptions() {
  ...
  optionsCheck(
    ...
    sourcesList.find(
      (element) => element.name == awLanguage.querySelector("span").textContent
    ).list
  );
  optionsCheck(
    ...
    sourcesList
      .find(
        (element) =>
          element.name == awLanguage.querySelector("span").textContent
      )
      .list.find(
        (element) =>
          element.name == awCategory.querySelector("span").textContent
      ).list
  );
}
...
function addSites() {
  ...
  const sources = sourcesList
    ...
    .list.find(
      (element) => element.name == awCategory.querySelector("span").textContent
    )
    .list.find(
      (element) => element.name == awSource.querySelector("span").textContent
    ).list;
...
function reqCall_Data() {
  document.querySelector("#aw-aliases").querySelector("span").textContent =
    "Loading...";
...
function reqCall_Episode() {
  fetch(
    `https://api.trakt.tv/${aw_data.type}/${aw_data.id}/seasons/${aw_data.season}/episodes/${aw_data.episode}?extended=full`,
    { method: "GET", headers: traktApiHeaders }
  )
    .then((response) => response.json())
    .then((data) => {
      data.number_abs
